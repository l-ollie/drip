...
function lowDrip(width, height, i) {
	...
	function setAttributeNSForSvg(createElement, attributes, isPath) {
		...
		const _singlePath = `M ${i * width} 0 v ${height} a ${halfWidth} ${halfWidth} ${arc} ${width} 0 v -${height} Z`;
...
container.onmousedown = function(e) {
	...
	function changeAnimation() {
		...
		const regexPath = /M+ +[0-9]+ +[0-9]+ +v+ +[0-9]+ +a+ +[0-9]+ +[0-9]+ +[0-9]+ +[0-9]+ +[0-9]+ +[0-9]+ +[0-9]+ +v+ +-+[0-9]+ +Z/;
		let svgAnimation = document.getElementById(`dripAnim${animId}`);
		const valuesProp = svgAnimation.getAttributeNS(null, 'values');
		let values;

		while ((values = regexPath.exec(valuesProp)) !== null) {
			console.log(`Found ${values[0]}`);
		}
		// = valuesProp.Matches(regexPath);

		const changedValue = valuesProp.replace(values[0], `${addLength(values[0], dragLength)}`);
